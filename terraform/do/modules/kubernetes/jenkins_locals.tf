...
// Also includes naming conventions, pod templates, containers configuration, and init groovy scripts.
  ...
  jenkins = {
    ...
    jenkins = {
      request_cpu = coalesce(
        ...
        try(var.resources_config[local.jenkins.presets.jenkins].requests.cpu, "500m")
      )
      request_memory = coalesce(
        var.jenkins_request_memory,
        try(var.resources_config[local.jenkins.presets.jenkins].requests.memory, "512Mi")
      )
      limit_cpu = coalesce(
        var.jenkins_limit_cpu,
        try(var.resources_config[local.jenkins.presets.jenkins].limits.cpu, "2000m")
      )
      limit_memory = coalesce(
        var.jenkins_limit_memory,
        try(var.resources_config[local.jenkins.presets.jenkins].limits.memory, "2048Mi")
      )
    }
    volume_permissions = {
      request_cpu = coalesce(
        var.jenkins_volume_permissions_request_cpu,
        try(var.resources_config[local.jenkins.presets.volume_permissions].requests.cpu, "16m")
      )
      request_memory = coalesce(
        var.jenkins_volume_permissions_request_memory,
        try(var.resources_config[local.jenkins.presets.volume_permissions].requests.memory, "32Mi")
      )
      limit_cpu = coalesce(
        var.jenkins_volume_permissions_limit_cpu,
        try(var.resources_config[local.jenkins.presets.volume_permissions].limits.cpu, "128m")
      )
      limit_memory = coalesce(
        var.jenkins_volume_permissions_limit_memory,
        try(var.resources_config[local.jenkins.presets.volume_permissions].limits.memory, "256Mi")
      )
    }
    agent = {
      request_cpu = coalesce(
        var.jenkins_agent_request_cpu,
        try(var.resources_config[local.jenkins.presets.agent].requests.cpu, "100m")
      )
      request_memory = coalesce(
        var.jenkins_agent_request_memory,
        try(var.resources_config[local.jenkins.presets.agent].requests.memory, "128Mi")
      )
      limit_cpu = coalesce(
        var.jenkins_agent_limit_cpu,
        try(var.resources_config[local.jenkins.presets.agent].limits.cpu, "500m")
      )
      limit_memory = coalesce(
        var.jenkins_agent_limit_memory,
        try(var.resources_config[local.jenkins.presets.agent].limits.memory, "512Mi")
      )
    }
    name = "jenkins"
    ...
    containers = {
      kani_interface = {
        ...
        pipeline_script = templatefile("${path.module}/scripts/jenkins/jenkinsfiles/deployment-rollout.jenkinsfile", {
          ...
          namespace = "kani"
          inherit_from = local.jenkins.pod_templates["deployment-rollout-jenkins-agent"].name
        })
    ...
    init_groovy_cm_1 = {
      "kubernetes-cloud.groovy" = templatefile("${path.module}/scripts/jenkins/groovy/kubernetes-cloud.groovy", {
        ...
        deployment_rollout_agent_yaml = local.jenkins.pod_templates["deployment-rollout-jenkins-agent"].yaml,
        container_cap    = var.jenkins_agent_container_cap,
        service_account  = kubernetes_service_account.jenkins_agent.metadata[0].name,
        pod_template_name = local.jenkins.pod_templates["deployment-rollout-jenkins-agent"].name
      })
    }
    init_groovy_cm_2 = {
      for job in local.jenkins.containers :
    ...
    init_groovy_cm = merge(local.jenkins.init_groovy_cm_1, local.jenkins.init_groovy_cm_2)
  }
}
