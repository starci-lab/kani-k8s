  ...
  kani_observer = {
    ...
    kani_observer = {
      request_cpu = coalesce(
        ...
        try(var.resources_config[local.kani_observer.presets.kani_observer].requests.cpu, "100m")
      )
      request_memory = coalesce(
        var.kani_observer_request_memory,
        try(var.resources_config[local.kani_observer.presets.kani_observer].requests.memory, "128Mi")
      )
      limit_cpu = coalesce(
        var.kani_observer_limit_cpu,
        try(var.resources_config[local.kani_observer.presets.kani_observer].limits.cpu, "200m")
      )
      limit_memory = coalesce(
        var.kani_observer_limit_memory,
        try(var.resources_config[local.kani_observer.presets.kani_observer].limits.memory, "256Mi")
      )
    }
    name = "kani-observer"
