  ...
  kani_coordinator = {
    ...
    kani_coordinator = {
      request_cpu = coalesce(
        ...
        try(var.resources_config[local.kani_coordinator.presets.kani_coordinator].requests.cpu, "100m")
      )
      request_memory = coalesce(
        var.kani_coordinator_request_memory,
        try(var.resources_config[local.kani_coordinator.presets.kani_coordinator].requests.memory, "128Mi")
      )
      limit_cpu = coalesce(
        var.kani_coordinator_limit_cpu,
        try(var.resources_config[local.kani_coordinator.presets.kani_coordinator].limits.cpu, "200m")
      )
      limit_memory = coalesce(
        var.kani_coordinator_limit_memory,
        try(var.resources_config[local.kani_coordinator.presets.kani_coordinator].limits.memory, "256Mi")
      )
    }
