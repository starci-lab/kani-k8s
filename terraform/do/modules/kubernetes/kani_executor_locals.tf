  ...
  kani_executor = {
    ...
    kani_executor = {
      request_cpu = coalesce(
        ...
        try(var.resources_config[local.kani_executor.presets.kani_executor].requests.cpu, "100m")
      )
      request_memory = coalesce(
        var.kani_executor_request_memory,
        try(var.resources_config[local.kani_executor.presets.kani_executor].requests.memory, "128Mi")
      )
      limit_cpu = coalesce(
        var.kani_executor_limit_cpu,
        try(var.resources_config[local.kani_executor.presets.kani_executor].limits.cpu, "200m")
      )
      limit_memory = coalesce(
        var.kani_executor_limit_memory,
        try(var.resources_config[local.kani_executor.presets.kani_executor].limits.memory, "256Mi")
      )
    }
    name = "kani-executor"
