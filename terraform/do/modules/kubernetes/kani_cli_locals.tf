  ...
  kani_cli = {
    ...
    kani_cli = {
      request_cpu = coalesce(
        ...
        try(var.resources_config[local.kani_cli.presets.kani_cli].requests.cpu, "100m")
      )
      request_memory = coalesce(
        var.kani_cli_request_memory,
        try(var.resources_config[local.kani_cli.presets.kani_cli].requests.memory, "128Mi")
      )
      limit_cpu = coalesce(
        var.kani_cli_limit_cpu,
        try(var.resources_config[local.kani_cli.presets.kani_cli].limits.cpu, "200m")
      )
      limit_memory = coalesce(
        var.kani_cli_limit_memory,
        try(var.resources_config[local.kani_cli.presets.kani_cli].limits.memory, "256Mi")
      )
    }
    name = "kani-cli"
